  ...
  if (ggc_test_and_set_mark (x))
    {
      ...
      gt_ggc_m_P13libfunc_entry4htab ((*x).x_libfunc_hash);
    }
}
  ...
  if (gt_pch_note_object (x, x, gt_pch_p_15target_libfuncs))
    {
      ...
      gt_pch_n_P13libfunc_entry4htab ((*x).x_libfunc_hash);
    }
}
...
void * ggc_alloc_splay_tree_mips16_local_alias_htab (int sz, void * nl)
{
  return ggc_splay_alloc (sz, nl);
}

void * ggc_alloc_splay_tree_mflip_mips16_entry_htab (int sz, void * nl)
{
  return ggc_splay_alloc (sz, nl);
}
...
EXPORTED_CONST struct ggc_root_tab gt_ggc_r_gtype_desc_c[] = {
  ...
  },
  {
    &mips16_globals,
    1,
    sizeof (mips16_globals),
    &gt_ggc_mx_target_globals,
    &gt_pch_nx_target_globals
  },
  {
    &line_table,
    1,
